...
import com.chat.cs6650assignment2.model.ServerResponse;
...
public class ChatWebSocketHandler  extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        try {
            // Extract the room ID from the connection URI
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        try {
            ...
            System.out.println("Successfully published message to " + queueMessage.getRoomId() + " " + queueMessage.getMessageId());
            ...
            try {
                ...
                // If sending the error fails, there's not much more we can do.
            }
        }
    }
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
        ...
        if (roomId != null) {
            ...
            // This might happen if the connection failed before the room ID was set.
            System.out.println("Connection closed from " + session.getRemoteAddress() + " (room ID unknown) with status " + status);
        }
    }

    // The getRoomId helper method is fine, but let's make it a bit safer.
    private String getRoomId(WebSocketSession session) {
        ...
        if (lastSlash == -1 || lastSlash == path.length() - 1) {
            return null; // No slash found or it's the last character
        }
